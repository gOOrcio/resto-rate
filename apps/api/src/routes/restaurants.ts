...
import { db } from '../db';
import { restaurant, category, restaurantCategory, review, user, type CreateRestaurantRequest, type RestaurantResponse } from '../db/schema';
import { eq, desc, sql } from 'drizzle-orm';
import { requireAuth, optionalAuth } from '../middleware/auth';
import { generateRestaurantId, generateId } from '../lib/ulid';
...
export const restaurantRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', { preHandler: [optionalAuth] }, async (request, reply) => {
    const { limit = 20, offset = 0, category: categorySlug } = request.query as { 
      limit?: number; 
      offset?: number; 
      category?: string; 
    };
  ...
  fastify.post<{ Body: CreateRestaurantRequest }>('/', { preHandler: [requireAuth] }, async (request, reply) => {
    const { 
      name, 
      description, 
      cuisineType, 
